...
# 5、需要2个参数，第1个为hostname.txt文件，第2个为子网掩码（Requires 2 parameters, the first is the hostname.txt file, and the second is the subnet mask）
# ###
...
ssh -f root@${ip} "sed \"12 arestrict ${gateway_addr} mask ${net_mask} nomodify notrap\" -i /etc/ntp.conf;sed \"12 arestrict ${ip} nomodify notrap nopeer noquery\" -i /etc/ntp.conf"

ssh -f root@${ip} "sed \"s/server/#server/\" -i /etc/ntp.conf"

ssh -f root@${ip} "sed \"26 aFudge ${ip_addr} stratum 10\" -i /etc/ntp.conf;sed \"26 aserver ${ip_addr}\" -i /etc/ntp.conf"
done < ${filename}
