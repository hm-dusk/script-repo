...
expect << EOF
  spawn ssh root@${ip1} "echo ${ip2} ${host2} ${sort2} >> /etc/hosts"
  expect {
  "yes/no" {send "yes\r"; exp_continue}
  "password" {send "${pwd1}\r"}
  }
...
expect << EOF
  spawn ssh root@${sort} "ssh-keygen -t rsa -P '' -f ~/.ssh/id_rsa"
  expect {
  "yes/no" {send "yes\r"; exp_continue}
  "password" {send "${pwd}\r"}
  }
...
# 复制节点的key文件到本地
expect << EOF
  spawn scp ${sort}:/root/.ssh/id_rsa.pub ./${sort}.pub
  expect {
  "yes/no" {send "yes\r"; exp_continue}
  "password" {send "${pwd}\r"}
  }
...
cat ${filename} | while read ip host sort pwd; do
expect << EOF
  spawn scp ./authorized_keys ${sort}:/root/.ssh/
  expect {
  "yes/no" {send "yes\r"; exp_continue}
  "password" {send "${pwd}\r"}
  }
